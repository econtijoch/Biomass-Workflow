individual_scale <- function(biom_file, metadata_file, taxonomy_file, filter) {
	...
	raw_biom_table <- read_biom(biom_file)
	biom_data <- as(biom_data(raw_biom_table), 'matrix')
	...
	biom_only <- biom_only[biom_only$X.SampleID %in% intersect(biom_only$X.SampleID, metadata$X.SampleID),] %>% dplyr::arrange(X.SampleID)
	metadata <- metadata[metadata$X.SampleID %in% intersect(biom_only$X.SampleID, metadata$X.SampleID),] %>% dplyr::arrange(X.SampleID)
	
	cat('Reading Taxonomy...\n')
	# Read in Taxonomy
	taxonomy <- read.delim(taxonomy_file, header = F)
	...
	# Add metadata
	#biom_merged <- inner_join(biom_only, biomass, by = "X.SampleID")
	
	# Work with specific components
	otus_only <- biom_only[,-length(names(biom_only))]
	#metadata_only <-  biom_merged[, !grepl("OTU_", names(biom_merged))]
	#metadata_only$X.SampleID <- as.character(metadata_only$X.SampleID)
	
	cat('Scaling taxonomic abundances...\n')
	# Create relative abundances and scale
	pre_normalized <- otus_only/rowSums(otus_only)
	...
	melted <- melt(data = scaled, id.vars = 'X.SampleID', measure.vars = colnames(scaled)[-length(colnames(scaled))])
	...
	relative_melted <- melt(data = relative, id.vars = 'X.SampleID', measure.vars =  colnames(relative)[-length(colnames(relative))])
	...
	relative_split_tax <- relative_taxonomy_collapsed %>% tidyr::separate(Taxon, phylogeny, sep = ";.__", remove = F, fill = 'right')
	...
	for (i in 1:length(phylogeny)) {
		...
		table <- split_tax %>% group_by(X.SampleID) %>% group_by_(.dots = dots, add = T) %>% dplyr::summarise(abundance = sum(absolute_abundance))
		if (i > 1) {
		...
		pre_compact_scaled <- dcast(table, X.SampleID + Depth + Abundance_Type ~ long_label, value.var = 'abundance')
		compact_by_tax_scaled[[phylogeny[i]]] <- dplyr::left_join(metadata, pre_compact_scaled, by = "X.SampleID")
		
		
		## Relative Abundance
		
		rel_table <- relative_split_tax %>% group_by(X.SampleID) %>% group_by_(.dots = dots, add = T) %>% dplyr::summarise(abundance = sum(relative_abundance))
		if (i > 1) {
		...
		pre_compact_relative <- dcast(rel_table, X.SampleID + Depth + Abundance_Type ~ long_label, value.var = 'abundance')
		compact_by_tax_relative[[phylogeny[i]]] <- dplyr::left_join(metadata, pre_compact_relative, by = "X.SampleID")
	}
	
	
	return(list(scaled_otus = otus_scaled, relative_otus = otus_relative, melted_scaled_by_taxonomy = melted_by_tax_scaled, melted_relative_by_taxonomy = melted_by_tax_relative, compact_scaled_by_taxonomy = compact_by_tax_scaled, compact_relative_by_taxonomy = compact_by_tax_relative))
