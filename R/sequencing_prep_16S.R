#' Function to produce files necessary to generate 16S sequencing runs that utilizes the Beckmann robot for dilutions and creates sequencing mapping files in QIIME-ready format, from multiple experiment data sets.
...
sequencing_prep_16S <- function(experiment_data_list, n_barcode_plates, barcode_map_file = NULL, output_directory = getwd(), 
    filter = T) {
    ...
    for (i in 1:n_sequencing_runs) {
        ...
        sequencing_runs[[sequencing_id]][["Mapping"]] <- suppressWarnings(dplyr::filter(sequencing_combined, BarcodeID %in% samples) %>% 
            ...
            BarcodeSequence, LinkerPrimerSequence, SequencingRun, BarcodePlate, BarcodeWell, dplyr::everything()) %>% dplyr::mutate(Description = SampleID))
